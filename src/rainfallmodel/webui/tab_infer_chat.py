...
def create_infer_chat_tab(manager: "Manager") -> dict[str, "Component"]:
    ...
    load_model_btn.click(manager.infer_chat_runner.load_model, inputs=input_elems, outputs=[info_box])
    unload_model_btn.click(manager.infer_chat_runner.unload_model, inputs=[], outputs=[info_box])
    ...
    submit_btn.click(manager.infer_chat_runner.interface_chat, generate_inputs, [chatbot, messages, query])
