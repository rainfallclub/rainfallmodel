...
def get_lora_config(sft_conf:dict):
    lora_config = LoraConfig(
        r= int(sft_conf['lora_rank']),
        lora_alpha=int(sft_conf['lora_alpha']),
        target_modules=sft_conf['lora_target_modules'],
        lora_dropout=float(sft_conf['lora_dropout']),
        bias="none",
        task_type="CAUSAL_LM"
    )
    return lora_config
